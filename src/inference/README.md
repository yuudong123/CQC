...
응답에는 요청과 동일한 `inspection_id`, 실제 사용한 `used_frame_count`, 품종·품질별 확률, 예측 클래스, 두 신뢰도, 순수 모델 추론시간, 모델명과 버전이 포함된다. 요청 이미지는 승인 모델 입력 장수인 12장 이하, 합계 24MiB까지 허용한다. 12장보다 적으면 마스크 패딩하며, 초과 입력은 HTTP 413으로 거부한다.
