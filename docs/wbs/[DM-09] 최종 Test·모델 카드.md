...
## 2026-09-26 시연 배차 정책 변경

v2 외관 모델은 유지한다. 정상 배차는 v2 품종 2종 × 외관 3단계 × 가상 당도 12°Brix 미만/이상 2단계의 **12개 bin**으로 변경한다. 재검사 bin 1개는 유지한다. 아래 2026-09-23의 60:40 `commercial_grade`는 후속 학습 실험 및 별도 종합등급 표시의 기록일 뿐 **새 배차 키가 아니다**. 구현·잔여 연동 범위는 [12-bin 인계](협업공지/12-bin-가상당도-배차-인계.md)를 따른다.
...
- 5-fold Group CV는 집 PC `outputs/training-commercial-v3-cv`에서 완료됐다. Test 27그룹을 제외한 개발 사과 152개를 그룹 단위로 분리하고, 사전에 정한 공통 epoch 17을 비교했다. 합성 종합등급 Macro F1은 fold별 **0.7336·0.7343·0.7030·0.3592·0.6114**, 평균 **0.6283**(최저 0.3592)이다. 품종 Macro F1 평균은 **0.9934**다. [원본 보고서](results/commercial-v3-group-cv.json).
- v3의 source 검증 0.6238과 개발 CV 평균 0.6283은 모두 합성 종합등급에 대한 값이다. fold 간 편차가 크고 v2+60:40 규칙의 기존 Test 1.0000과는 평가 분할·학습 사과 수가 달라 동등한 모델 우열 수치로 취급하지 않는다. v3를 종합등급 출력 모델로 확정할 강건한 근거는 부족하다. 통합 모델은 **v2 외관 모델**로 유지하고, 60:40 종합등급은 별도 실험·표시 정책으로 보존한다. 실제 시연 배차는 2026-09-26에 정한 12-bin 정책을 따른다.
