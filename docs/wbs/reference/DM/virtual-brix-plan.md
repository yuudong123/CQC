...
> 2026-09-26 변경: 아래 60:40 종합등급은 실험·표시용으로 보존한다. 실제 시연 배차는 **v2 품종·외관 × 가상 당도 12°Brix 미만/이상 = 정상 12 bin**이며 종합등급으로 배차하지 않는다. 상세 매핑과 통합 상태는 [12-bin 인계](../../협업공지/12-bin-가상당도-배차-인계.md)를 따른다.
...
2026-09-23 개발 실험 집계에서 이미지 단독 A의 강건 점수는 0.79316, 결합 B는 0.76398이었다. 현재 최종 학습 후보는 A(separate·12장·focal·4 epoch)이며 가상 당도는 별도 시연값으로 유지한다. 이는 최종 품질 승인 결과가 아니다. C는 생성값을 다시 예측하는 순환 학습이라 후순위다. 가상 당도 API·화면 연결은 아직 완료되지 않았다.
