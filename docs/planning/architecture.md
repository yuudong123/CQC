...
sequenceDiagram
    ...
    BE->>CTL: 정상 12개 bin 또는 재검사 bin 명령
    CTL-->>BE: 제어 결과
    BE->>DB: 검사 결과 저장
    FE->>BE: 상태·이력·통계 조회
...
스마트폰 카메라, 사용자 이미지 파일 업로드와 실제 산업용 카메라는 사용하지 않는다. 입력 시뮬레이터는 `data/processed/realtime-apple-arrival-demo/index.json`에서 `default_playback=true`인 사전 구성 12장 묶음을 순서대로 읽고, 묶음별 multipart 요청을 500ms 간격으로 전송하며 기본 상태는 ON이다. `partial_groups`의 1~11장 묶음은 기본 재생에서 제외하고 누락 뷰 시연 때만 별도로 사용한다. 사용자는 시연 화면에서 시작·정지할 수 있다.

입력 이미지는 목록 순서대로 사용한다. 정지는 새 입력만 차단하고 처리 중인 요청은 완료한다. 재시작하면 저장된 입력 위치부터 이어서 처리한다. 대시보드 집계는 오늘을 기본 기간으로 하며 1초마다 갱신한다.

입력 소스는 기존 사과의 학습 입력에서 제외한 촬영 사진을 재조합한 시연용 묶음이며 독립 Test나 새 사과 데이터가 아니다. 목록 끝에서는 처음으로 돌아간다. Docker 서비스가 시작되면 웹 접속과 무관하게 자동 실행한다. Backend가 Inference 요청을 전송한 시점부터 응답 전체를 수신할 때까지 500ms를 넘으면 재검사 bin 명령을 확정하며, 이후 도착한 결과가 물리적 분류 명령을 변경할 수 없다. 병렬 처리 수는 배포 CPU 성능시험으로 정한다.
...
| 입력 시뮬레이터 | 시연용 `index.json` 순회·500ms 간격 전송·시작/정지·위치 관리 | 홍준희 | 시연 전용 묶음·제어 명령 | Backend 검사 요청 |
| 웹 애플리케이션 | 자동 검사 상태, 결과·이력·통계 표시 | 강성민 | API 응답·시작/정지 명령 | 사용자 화면 |
| 백엔드 API | 입력 검증, 모델 호출, 정책, 조회 | 홍준희 | 웹 요청·모델 출력 | 검사·이력·통계 응답 |
| 판정 정책 | 두 신뢰도·제한시간과 가상 당도 구간으로 선별 목적지 결정 | 홍준희·조현재 | 품종·외관 확률, 처리시간, 시연용 가상 당도 | 정상 12개 bin 또는 재검사 bin |
