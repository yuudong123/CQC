...
실제 카메라는 사용하지 않는다. 입력 시뮬레이터가 저장된 사과 이미지를 초당 2장씩 지속적으로 전송하며 기본 상태는 ON이다. 사용자는 시연 화면에서 시작·정지할 수 있다. 제한 시간 내 처리하지 못한 입력은 대기열 적체나 입력 감속 없이 재검사 bin으로 보낸다.
...
입력은 500ms 간격, 모델 제한시간은 500ms, 가상 제어 응답 목표는 100ms로 둔다. 개별 흐름은 약 600ms가 걸릴 수 있으므로 파이프라인을 겹쳐 실행하여 초당 2건을 유지하며, 실제 동시 처리 수는 CPU 시험으로 확정한다.
