...
sequenceDiagram
    ...
    SIM->>BE: 최대 40장 다각도 사과 그룹 multipart 요청
    BE->>BE: 그룹·파일·요청 검증
...
스마트폰 카메라, 사용자 이미지 파일 업로드와 실제 산업용 카메라는 사용하지 않는다. 입력 시뮬레이터가 저장된 사과 그룹에서 균등 선택한 최대 40장의 다각도 이미지를 500ms 간격으로 전송하며 기본 상태는 ON이다. 40장 미만 그룹은 보유 이미지를 전송하고 Inference가 누락 뷰를 masking한다. 사용자는 시연 화면에서 시작·정지할 수 있다.
...
| HTTP 요청 경로·세부 OpenAPI·late result 수신 방식 | 홍준희·조현재 | 확정된 500ms 범위와 서비스 책임 경계 |
